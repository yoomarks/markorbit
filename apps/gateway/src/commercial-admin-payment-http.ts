...
export interface GatewayCommercialAdminPaymentOptions {
  paymentUrl?: string;
...
export function createGatewayCommercialAdminPaymentRoutes(
  options: GatewayCommercialAdminPaymentOptions
): readonly JsonRoute[] {
  ...
  const forward = async (
    ...
    try {
      const response = await fetch(
        `${paymentUrl}/internal/commercial-admin/payments/${encodeURIComponent(request.params.paymentId!)}`,
        {
          headers: {
