...
import { createGatewayExecutionSuperAdminRoutes } from '../src/execution-super-admin-http.js';

const secret = 'execution-super-admin-secret';
const coreUrl = 'http://core.test';
const executionUrl = 'http://execution.test';
...
function request(cookie = 'mo_session=browser-execution-admin-session'): JsonRequest {
  return {
    method: 'GET',
...
function route(fetchImpl: typeof fetch) {
  const found = createGatewayExecutionSuperAdminRoutes({
    coreUrl,
    executionUrl,
    internalServiceSecret: secret,
    fetchImpl
  })[0];
  if (!found) throw new Error('Missing Gateway Execution Admin route.');
  return found;
}

describe('Gateway Execution Super Admin read', () => {
      ...
      (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
        ...
        if (calls.length === 1) {
          expect(url(input)).toBe(
            `${coreUrl}/internal/super-admin/execution/operator-principals/resolve`
          );
          expect(body(init)).toEqual({ token: 'browser-execution-admin-session' });
          return response(principal);
        }
        expect(url(input)).toBe(`${executionUrl}/internal/super-admin/execution`);
  ...
  it('fails closed on malformed owner success and preserves owner non-2xx', async () => {
    ...
    await expect(route(malformedFetch).handle(request())).rejects.toMatchObject({
      status: 503,
      code: 'EXECUTION_ADMIN_OWNER_CONTRACT_MISMATCH'
    });
