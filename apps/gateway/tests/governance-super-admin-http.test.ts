...
import { createGatewayGovernanceSuperAdminRoutes } from '../src/governance-super-admin-http.js';

const secret = 'governance-super-admin-secret';
const coreUrl = 'http://core.test';
const ownerUrl = 'http://core.test';
const principal: InternalOperatorPrincipal = {
  kind: 'INTERNAL_OPERATOR',
  sessionId: 'session-governance-admin',
...
const owner = {
  ...
  portfolio: {
    ...
    reason: 'No canonical cross-owner Governance and Audit portfolio.'
  }
};

function request(cookie = 'mo_session=browser-governance-admin-session'): JsonRequest {
  return {
    method: 'GET',
...
function route(fetchImpl: typeof fetch) {
  const found = createGatewayGovernanceSuperAdminRoutes({
    coreUrl,
    internalServiceSecret: secret,
    fetchImpl
  })[0];
  if (!found) throw new Error('Missing Gateway Governance Admin route.');
  return found;
}

describe('Gateway Governance Super Admin read', () => {
      ...
      (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
        ...
        if (calls.length === 1) {
          expect(url(input)).toBe(
            `${coreUrl}/internal/super-admin/governance/operator-principals/resolve`
          );
          expect(body(init)).toEqual({ token: 'browser-governance-admin-session' });
          return response(principal);
        }
        expect(url(input)).toBe(`${ownerUrl}/internal/super-admin/governance`);
  ...
  it('fails closed on malformed owner success and preserves owner non-2xx', async () => {
    ...
    await expect(route(malformedFetch).handle(request())).rejects.toMatchObject({
      status: 503,
      code: 'GOVERNANCE_ADMIN_OWNER_CONTRACT_MISMATCH'
    });
