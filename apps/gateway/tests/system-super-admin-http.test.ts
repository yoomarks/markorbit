...
import { createGatewaySystemSuperAdminRoutes } from '../src/system-super-admin-http.js';

const secret = 'system-super-admin-secret';
const coreUrl = 'http://core.test';
const ownerUrl = 'http://core.test';
const principal: InternalOperatorPrincipal = {
  kind: 'INTERNAL_OPERATOR',
  sessionId: 'session-system-admin',
...
function request(cookie = 'mo_session=browser-system-admin-session'): JsonRequest {
  return {
    method: 'GET',
...
function route(fetchImpl: typeof fetch) {
  const found = createGatewaySystemSuperAdminRoutes({
    coreUrl,
    internalServiceSecret: secret,
    fetchImpl
  })[0];
  if (!found) throw new Error('Missing Gateway System Admin route.');
  return found;
}

describe('Gateway System Super Admin read', () => {
      ...
      (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
        ...
        if (calls.length === 1) {
          expect(url(input)).toBe(
            `${coreUrl}/internal/super-admin/system/operator-principals/resolve`
          );
          expect(body(init)).toEqual({ token: 'browser-system-admin-session' });
          return response(principal);
        }
        expect(url(input)).toBe(`${ownerUrl}/internal/super-admin/system`);
  ...
  it('fails closed on malformed owner success and preserves owner non-2xx', async () => {
    ...
    await expect(route(malformedFetch).handle(request())).rejects.toMatchObject({
      status: 503,
      code: 'SYSTEM_ADMIN_OWNER_CONTRACT_MISMATCH'
    });
