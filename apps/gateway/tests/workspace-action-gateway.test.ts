...
function workspaceActions(empty = false) {
  const currentItem = {
    formalMatter: {
...
async function stack(
  input: { ownerStatus?: number; ownerBody?: unknown; networkFailure?: boolean } = {}
) {
        ...
        routes: [
          {
            ...
            handle: (request) => {
              ...
              return json(
                input.ownerStatus ?? 200,
                input.ownerBody ?? { workspaceActions: workspaceActions() }
              );
            }
          }
        ]
...
describe('authenticated Workspace Action Projection V1 Gateway read', () => {
  it('forwards current and successful-empty owner truth without reconstructing it', async () => {
    const base = await stack();
    const response = await fetch(endpoint(base), { headers: browserHeaders() });
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ workspaceActions: workspaceActions() });

    const emptyBase = await stack({ ownerBody: { workspaceActions: workspaceActions(true) } });
    const empty = await fetch(endpoint(emptyBase), { headers: browserHeaders() });
    expect(empty.status).toBe(200);
    expect(await empty.json()).toEqual({ workspaceActions: workspaceActions(true) });
  });

  it('derives downstream authority server-side and ignores browser internal-header spoofing', async () => {
    const base = await stack();
  ...
  it('preserves owner 404 and retryable 503 instead of fabricating empty Workspace truth', async () => {
    for (const [status, code] of [
      [404, 'WORKSPACE_MISMATCH'],
      [503, 'WORKSPACE_ACTION_TRUTH_UNAVAILABLE']
    ] as const) {
      const base = await stack({
        ownerStatus: status,
        ownerBody: { code, message: 'Owner truth unavailable.', retryable: status === 503 }
      });
      const response = await fetch(endpoint(base), { headers: browserHeaders() });
      expect(response.status).toBe(status);
      const body: unknown = await response.json();
      expect(body).toMatchObject({ code });
      expect(body).not.toHaveProperty('workspaceActions');
    }
  });

  it('fails closed on MarkReg network failure and exposes no mutation route', async () => {
    const networkBase = await stack({ networkFailure: true });
    const unavailable = await fetch(endpoint(networkBase), { headers: browserHeaders() });
