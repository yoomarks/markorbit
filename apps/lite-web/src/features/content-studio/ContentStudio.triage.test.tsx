...
describe('Content Studio action-first triage', () => {
  it('derives triage only from current exact Draft and Package lineage', () => {
    ...
    expect(contentWorkTriage(work(3, 'Review', 'READY_FOR_HUMAN_REVIEW'))).toBe(
      'READY_FOR_REVIEW'
    );
    ...
    expect(
      contentWorkTriage(work(6, 'Package present', 'REVIEWED_READY_FOR_PACKAGE', true))
    ).toBe('PACKAGE_PRESENT');
  ...
  it(
    'filters and searches only the loaded owner work while keeping next-focus guidance',
    async () => {
      const items = [
      ...
      render(
        <ContentStudio
          ...
          client={fixtureClient(listFixture(items))}
        />
      );
      ...
      await userEvent.selectOptions(
        screen.getByLabelText('Current work state'),
        'READY_FOR_REVIEW'
      );
