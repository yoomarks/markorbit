...
it('renders loading, stale, and recoverable repository errors through the app', async () => {
  ...
  const { rerender } = render(<LiteApp initialSurface="customers" repository={never} />);
  expect(screen.getByText('Loading fixture workspace…')).toBeVisible();
  rerender(<LiteApp initialSurface="customers" fixtureState="stale" />);
  ...
  rerender(<LiteApp initialSurface="customers" repository={broken} />);
