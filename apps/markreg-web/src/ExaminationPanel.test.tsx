...
describe('ExaminationPanel', () => {
  it.each(workflowCases)('renders deterministic treatment for %s', async (state, label) => {
    ...
    expect(
      screen.getByText(/not trademark-office Official Status or deadline truth/i)
    ).toBeTruthy();
  ...
  it('keeps historical entries explicitly historical and separate from current workflow truth', async () => {
    ...
    expect(screen.getByLabelText('Truth class: Historical')).toBeTruthy();
    expect(
      screen.getByText(/Historical context is not current trademark-office status/i)
    ).toBeTruthy();
  });
