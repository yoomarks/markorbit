...
CI isolates destructive PostgreSQL verification in three databases on one PostgreSQL 16 service: persistence uses `markorbit_persistence_test` with migration namespace `persistence_foundation`, Identity uses `markorbit_identity_test` with `core_identity`, and Session authentication uses `markorbit_auth_test` with `core_auth`. Identity and authentication setup each drops the complete dependent Core table chain and their suite-local migration-history schema before reapplying immutable migrations. Both focused suites run twice consecutively.

The source-derived Gateway inventory has 60 entries: the historical 57-route Milestone set plus three production authentication routes. `POST /__test/auth/session` is excluded and documented separately as test-runtime-only. The existing six-process Milestone 1 topology does not exercise authentication routes and remains intentionally unchanged; its runtime and source-inventory checks are verified independently.
