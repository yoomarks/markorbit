...
**Status:** proposed. Each task is one bounded branch/PR. TASK 016 performs no implementation.

The provisional numbering is retained because repository dependencies support foundation → identity → auth → domain → persistence → service → transport → UI → reliability → audit. Audit/idempotency semantics are designed in TASK 017 and integrated atomically in TASK 022; TASK 025 hardens publication, denial audit and operational evidence rather than adding them too late.

## Staged rollout

| Stage/task | Dependency             | Deliverable and acceptance                                                                                                         | Rollback boundary                                                         | Durable / fixture records                                                                                                     |
| ---------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 1 / 017    | owner technology gates | PostgreSQL driver/migrations, transaction/repository contracts, DB test harness; empty bootstrap and memory/Postgres contract pass | remove adapter/migrations before data; forward repair after shared schema | foundation only; all M1 fixture                                                                                               |
| 2 / 018    | 017, terminology gate  | User/Workplace/Membership/role contracts and Core repository; tenant constraints/isolation tests                                   | identity schema + additive contracts                                      | identity durable; workflow fixture                                                                                            |
| 3 / 019    | 018, auth gate         | opaque sessions, principal, Gateway/service identity propagation, CSRF; login/logout/expiry/forgery tests                          | auth routes/store independently removable                                 | identity/session durable                                                                                                      |
| 4 / 020    | authority gates        | Formal Matter/source snapshot/command/error/event fixtures; authority tests                                                        | additive shared contract rollback while unused                            | no new runtime records                                                                                                        |
...
| 6 / 022    | 018–021                | authorized `CreateFormalMatter`; exact lineage admission; service contract tests                                                   | route/operation feature flag, retained compatible rows                    | Matter + snapshot durable; Confirmation/Draft durability completed here or prerequisite split if owner changes classification |
...
| 9 / 025    | 022–024, outbox gate   | append-only success/denial audit queries, idempotency operations and optional outbox worker/dedupe                                 | worker publication disabled; committed audit retained                     | audit/key/outbox durable                                                                                                      |
...
- **Dependencies:** TASK 016 owner gates. **Allowed:** infrastructure, config, service-kit/test-kit, per-service migration roots, package manifests/lockfile, tests/docs. **Prohibited:** auth, Formal Matter behavior, wholesale repository conversion.
...
### TASK 018 — Identity, Workplace and Membership contracts

- **Objective/owner:** Core owns durable User, Workplace, Membership, bounded role/permission mapping.
...
- **Evidence:** unique membership, inactive membership, two-Workplace isolation, memory/Postgres repository parity.
...
- **Objective/owner:** Core/Auth + Gateway establish opaque sessions, CSRF and tamper-resistant principal/service context.
- **Dependencies:** 018 and mechanism approval. **Allowed:** Core, Gateway, contracts/config, auth UI entry only, tests/docs. **Prohibited:** broad IAM, SSO, Formal Matter.
- **Evidence:** login/logout/expiry/rotation; secure cookie flags; CSRF; forged actor/Workplace rejection; fixture mode cannot enable production.
...
- **Dependencies:** 016 authority decisions, 018 identity terms. **Allowed:** contracts, MarkReg domain tests/fixtures/docs. **Prohibited:** repository, Gateway/UI, Order/Payment/Filing.
...
- **Objective/owner:** MarkReg implements memory/PostgreSQL repository adapters, snapshot, audit and idempotency transaction primitives.
...
- **Objective/owner:** MarkReg/Platform complete append-only audit access, denial records, key retention and approved outbox behavior.
- **Dependencies:** 022–024 and outbox gate. **Allowed:** MarkReg, Gateway audit read boundary, events, migrations/tests/docs. **Prohibited:** generic enterprise event platform or console logs as audit.
- **Evidence:** immutable audit fields, key reuse, restart replay, repeated publish/consumer dedupe, audit/outbox failure tests.
- **Rollback/size:** disable publisher; never delete committed audit/key records. Medium (450–850 lines).
