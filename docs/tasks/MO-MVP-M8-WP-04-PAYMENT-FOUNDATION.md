...
Core remains identity/workspace authority. Gateway remains the browser authentication, Workspace Principal, CSRF and trusted-Origin boundary.
...
- amount/currency derived exclusively from Checkout snapshot;
- no existing non-terminal Payment for the same Checkout unless the operation is an idempotent replay.

Payment creation snapshots Checkout ID, Order ID, Product/Price versions, amount and currency.
...
The first production provider is locked only when its current official API and webhook requirements have been reviewed. Provider-specific SDK types must not leak into shared business contracts.
...
- Browser payment initiation uses canonical HttpOnly Session through Gateway.
...
Migrations are restart-safe and registered to the Payment service owner.
...
- real provider sandbox/test-mode end-to-end path passes before WP04 completion.
