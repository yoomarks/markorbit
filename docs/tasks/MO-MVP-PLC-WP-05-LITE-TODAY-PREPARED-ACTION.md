...
## 18. Final static and replay closure

A concise hosted static diagnostic confirmed Lite, MarkReg and Gateway lint/typecheck were already clean and isolated the remaining Lite Web issues to UI type mapping only: business Recommendation/handoff states were being passed to the semantic-tone `StatusBadge`, and an optional `journey` prop was being passed explicitly as `undefined` under `exactOptionalPropertyTypes`. WP-05 now renders those business states as ordinary `Badge` content and omits the optional prop when no journey exists.

The durable confirmation replay path was also reviewed before final acceptance. `confirmAndHandoff()` now reloads the current durable journey after idempotent confirmation replay and returns an already-completed handoff without invoking the owner again. With the repository dependency build in place, the dedicated PostgreSQL Prepared Action suite passes all four durability scenarios: completed replay, confirmation-before-dependency-failure with safe retry, Workspace/stale-source isolation, and acknowledgement/consequence fail-closed behavior.

## 19. Exact-head CI shared-root closure

The `7ea8241e` exact-head run confirmed that the M4 reliability matrix itself passed and that legacy persistence validation remained intact. The remaining red checks reduced to two shared mechanical roots in WP-05: one selected-Recommendation detail still referenced the removed `StatusBadge`, which stopped Lite Web typecheck and left the real browser surface unable to render; and five new WP-05 runtime/test files had not been passed through repository Prettier, which failed the final `pnpm check` used by the reliability workflows. The stale component reference was replaced with the existing neutral business `Badge`, and the five files plus Today were formatted and rechecked before triggering the next exact-head run. No authority boundary, state transition, owner consequence or acceptance criterion was weakened to obtain green CI.

## 20. Authenticated HTTP harness closure

The next exact-head run passed WP-05 lint/typecheck, all four Prepared Action PostgreSQL durability cases and the desktop/mobile real-runtime browser journey, then exposed a test-harness-only module-resolution fault: a root-level Vitest file imported workspace aliases that are not linked from the root package. Aligning those imports with built shared package instances allowed the authenticated HTTP suite to execute. That exposed an important harness identity detail: `HttpError` must come from the same built `@markorbit/service-kit` instance used by Lite routes, otherwise a deliberate `WORKSPACE_MISMATCH` 404 is misclassified by a second runtime constructor as an unknown 500. The harness now imports the built shared package instances, and the PostgreSQL-backed authenticated HTTP suite passes both the golden path and fail-closed Workspace/permission/trust cases without changing Lite route semantics.
