import { BrainContractError, parseBrainEvidenceRef, type BrainConfidence, type BrainEvidenceRef } from './brain.js';
...
export function parseBrainConfidencePolicy(value: unknown): BrainConfidencePolicy {
  ...
  if (policy.schemaVersion !== 1) throw new BrainContractError('brainConfidencePolicy.schemaVersion must be 1.');
  ...
  const parsedWeights = Object.fromEntries(
    factorNames.map((factor) => [factor, unit(weights[factor], `brainConfidencePolicy.weights.${factor}`)])
  ...
  exactKeys(thresholds, ['low', 'medium', 'high', 'veryHigh'], 'brainConfidencePolicy.bandThresholds');
  ...
  if (!(bandThresholds.low < bandThresholds.medium && bandThresholds.medium < bandThresholds.high && bandThresholds.high < bandThresholds.veryHigh))
    throw new BrainContractError('brainConfidencePolicy.bandThresholds must be strictly increasing.');
  ...
  return {
    ...
    bandThresholds,
    freshnessHalfLifeDays: positive(policy.freshnessHalfLifeDays, 'brainConfidencePolicy.freshnessHalfLifeDays'),
    missingTimestampFreshness: unit(policy.missingTimestampFreshness, 'brainConfidencePolicy.missingTimestampFreshness'),
    singleSourceAgreement: unit(policy.singleSourceAgreement, 'brainConfidencePolicy.singleSourceAgreement'),
    counterEvidencePenalty: unit(policy.counterEvidencePenalty, 'brainConfidencePolicy.counterEvidencePenalty')
  };
}

export function parseBrainConfidenceQualityEvidence(value: unknown): BrainConfidenceQualityEvidence {
  const evidence = record(value, 'brainConfidenceQualityEvidence');
  exactKeys(evidence, [
    'coverage', 'validation', 'methodQuality', 'coverageReason', 'validationReason', 'methodQualityReason'
  ...
  return {
    ...
    validationReason: text(evidence.validationReason, 'brainConfidenceQualityEvidence.validationReason'),
    methodQualityReason: text(evidence.methodQualityReason, 'brainConfidenceQualityEvidence.methodQualityReason')
  };
}

export function parseBrainConfidenceEvaluationEvidenceRefs(value: unknown): readonly BrainEvidenceRef[] {
  if (!Array.isArray(value)) throw new BrainContractError('brainConfidenceEvaluation.evidenceRefs must be an array.');
