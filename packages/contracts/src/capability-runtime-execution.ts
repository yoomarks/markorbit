...
function evidenceRefs(value: unknown, field: string): readonly string[] {
  if (!Array.isArray(value) || value.length > 1000)
    throw new CapabilityRuntimeExecutionContractError(`${field} must be an evidence-ref array.`);
  const refs = value.map((item, index) => text(item, `${field}[${index}]`, 2000));
...
function parseRequest(value: unknown): CapabilityRequestV2 {
  ...
  return {
    ...command,
    capabilityRequestId: text(request.capabilityRequestId, 'request.capabilityRequestId', 300) as CapabilityRequestV2['capabilityRequestId'],
...
function parseEligibility(value: unknown): CapabilityEligibilityDecision {
  ...
  exactKeys(
    item,
    ['schemaVersion', 'capabilityRequestId', 'decision', 'eligible', 'policyVersion', 'reason', 'decidedAt'],
    'eligibility'
  );
  if (item.schemaVersion !== 1 || item.decision !== 'ELIGIBLE' || item.eligible !== true)
    throw new CapabilityRuntimeExecutionContractError('eligibility must be an accepted ELIGIBLE decision.');
  return {
    schemaVersion: 1,
    capabilityRequestId: text(item.capabilityRequestId, 'eligibility.capabilityRequestId', 300) as CapabilityEligibilityDecision['capabilityRequestId'],
    decision: 'ELIGIBLE',
    eligible: true,
...
function parseComposition(value: unknown): CapabilityComposition {
  ...
  if (item.schemaVersion !== 1 || item.mode !== 'SINGLE_IMPLEMENTATION')
    throw new CapabilityRuntimeExecutionContractError('composition must use SINGLE_IMPLEMENTATION.');
  if (!Array.isArray(item.supportingImplementationProfileIds) || !Array.isArray(item.criticImplementationProfileIds))
    throw new CapabilityRuntimeExecutionContractError('composition implementation lists are invalid.');
  return {
    schemaVersion: 1,
    capabilityRequestId: text(item.capabilityRequestId, 'composition.capabilityRequestId', 300) as CapabilityComposition['capabilityRequestId'],
    ...
    supportingImplementationProfileIds: item.supportingImplementationProfileIds.map((entry, index) =>
      text(entry, `composition.supportingImplementationProfileIds[${index}]`, 300)
    ) as CapabilityComposition['supportingImplementationProfileIds'],
    criticImplementationProfileIds: item.criticImplementationProfileIds.map((entry, index) =>
      text(entry, `composition.criticImplementationProfileIds[${index}]`, 300)
...
function parseBinding(value: unknown): ImplementationBinding {
  ...
  exactKeys(runtimeCapability, ['id', 'version', 'capabilityId', 'capabilityVersion'], 'binding.runtimeCapability');
  const implementation = record(item.implementation, 'binding.implementation');
  exactKeys(implementation, ['id', 'version', 'implementationKey', 'kind'], 'binding.implementation');
  if (
  ...
  return {
    schemaVersion: 1,
    implementationBindingId: text(item.implementationBindingId, 'binding.implementationBindingId', 300) as ImplementationBinding['implementationBindingId'],
    capabilityRequestId: text(item.capabilityRequestId, 'binding.capabilityRequestId', 300) as ImplementationBinding['capabilityRequestId'],
    runtimeCapability: {
      id: text(runtimeCapability.id, 'binding.runtimeCapability.id', 300) as ImplementationBinding['runtimeCapability']['id'],
      version: positiveInteger(runtimeCapability.version, 'binding.runtimeCapability.version'),
      capabilityId: text(runtimeCapability.capabilityId, 'binding.runtimeCapability.capabilityId', 300),
      capabilityVersion: text(runtimeCapability.capabilityVersion, 'binding.runtimeCapability.capabilityVersion', 120)
    },
    implementation: {
      id: text(implementation.id, 'binding.implementation.id', 300) as ImplementationBinding['implementation']['id'],
      version: positiveInteger(implementation.version, 'binding.implementation.version'),
      implementationKey: text(implementation.implementationKey, 'binding.implementation.implementationKey', 500),
      kind: implementation.kind as ImplementationBinding['implementation']['kind']
    },
    selectionPolicyVersion: text(item.selectionPolicyVersion, 'binding.selectionPolicyVersion', 300),
...
function parseInvocation(value: unknown): CapabilityInvocation {
  ...
  return {
    schemaVersion: 1,
    capabilityInvocationId: text(item.capabilityInvocationId, 'invocation.capabilityInvocationId', 300) as CapabilityInvocation['capabilityInvocationId'],
    capabilityRequestId: text(item.capabilityRequestId, 'invocation.capabilityRequestId', 300) as CapabilityInvocation['capabilityRequestId'],
    implementationBindingId: text(item.implementationBindingId, 'invocation.implementationBindingId', 300) as CapabilityInvocation['implementationBindingId'],
...
function parseOutcome(value: unknown): CapabilityOutcome {
  ...
  if (actual.some((key) => !allowed.includes(key)) || !allowed.filter((key) => key !== 'usage').every((key) => key in item))
    throw new CapabilityRuntimeExecutionContractError('outcome has an unsupported shape.');
  if (item.schemaVersion !== 1 || item.status !== 'SUCCEEDED' || item.output === undefined)
    throw new CapabilityRuntimeExecutionContractError('outcome must be a successful output-bearing result.');
  return {
    schemaVersion: 1,
    capabilityOutcomeId: text(item.capabilityOutcomeId, 'outcome.capabilityOutcomeId', 300) as CapabilityOutcome['capabilityOutcomeId'],
    capabilityRequestId: text(item.capabilityRequestId, 'outcome.capabilityRequestId', 300) as CapabilityOutcome['capabilityRequestId'],
    capabilityInvocationId: text(item.capabilityInvocationId, 'outcome.capabilityInvocationId', 300) as CapabilityOutcome['capabilityInvocationId'],
    status: 'SUCCEEDED',
    outputSchemaId: text(item.outputSchemaId, 'outcome.outputSchemaId', 300),
    output: structuredClone(item.output),
    evidenceRefs: evidenceRefs(item.evidenceRefs, 'outcome.evidenceRefs'),
    ...(item.usage === undefined ? {} : { usage: structuredClone(record(item.usage, 'outcome.usage')) }),
...
function parseReturn(value: unknown): CapabilityReturn {
  ...
  exactKeys(item, allowed, 'returnValue');
  ...
  return {
    schemaVersion: 1,
    capabilityReturnId: text(item.capabilityReturnId, 'returnValue.capabilityReturnId', 300) as CapabilityReturn['capabilityReturnId'],
    capabilityRequestId: text(item.capabilityRequestId, 'returnValue.capabilityRequestId', 300) as CapabilityReturn['capabilityRequestId'],
    capabilityOutcomeId: text(item.capabilityOutcomeId, 'returnValue.capabilityOutcomeId', 300) as CapabilityReturn['capabilityOutcomeId'],
    status: 'COMPLETED',
    outputSchemaId: text(item.outputSchemaId, 'returnValue.outputSchemaId', 300),
    output: structuredClone(item.output),
    evidenceRefs: evidenceRefs(item.evidenceRefs, 'returnValue.evidenceRefs'),
...
function parseReceipt(value: unknown): SessionReceipt {
  ...
  exactKeys(runtimeCapability, ['id', 'version', 'capabilityId', 'capabilityVersion'], 'receipt.runtimeCapability');
  ...
  return {
    schemaVersion: 1,
    sessionReceiptId: text(item.sessionReceiptId, 'receipt.sessionReceiptId', 300) as SessionReceipt['sessionReceiptId'],
    capabilityRequestId: text(item.capabilityRequestId, 'receipt.capabilityRequestId', 300) as SessionReceipt['capabilityRequestId'],
    ...
    runtimeCapability: {
      id: text(runtimeCapability.id, 'receipt.runtimeCapability.id', 300) as SessionReceipt['runtimeCapability']['id'],
      version: positiveInteger(runtimeCapability.version, 'receipt.runtimeCapability.version'),
      capabilityId: text(runtimeCapability.capabilityId, 'receipt.runtimeCapability.capabilityId', 300),
      capabilityVersion: text(runtimeCapability.capabilityVersion, 'receipt.runtimeCapability.capabilityVersion', 120)
    },
    implementation: {
      id: text(implementation.id, 'receipt.implementation.id', 300) as SessionReceipt['implementation']['id'],
      version: positiveInteger(implementation.version, 'receipt.implementation.version'),
      implementationKey: text(implementation.implementationKey, 'receipt.implementation.implementationKey', 500)
    },
    capabilityInvocationId: text(item.capabilityInvocationId, 'receipt.capabilityInvocationId', 300) as SessionReceipt['capabilityInvocationId'],
    capabilityOutcomeId: text(item.capabilityOutcomeId, 'receipt.capabilityOutcomeId', 300) as SessionReceipt['capabilityOutcomeId'],
    capabilityReturnId: text(item.capabilityReturnId, 'receipt.capabilityReturnId', 300) as SessionReceipt['capabilityReturnId'],
    evidenceRefs: evidenceRefs(item.evidenceRefs, 'receipt.evidenceRefs'),
...
export function parseGovernedCapabilityRuntimeExecutionV2(
  ...
  if (
    ...
    throw new CapabilityRuntimeExecutionContractError('Capability request linkage is inconsistent.');
  if (
    ...
    throw new CapabilityRuntimeExecutionContractError('Capability execution linkage is inconsistent.');
  if (
    ...
    throw new CapabilityRuntimeExecutionContractError('Capability binding and receipt are inconsistent.');
  if (
    ...
    throw new CapabilityRuntimeExecutionContractError('Capability result and receipt evidence are inconsistent.');
