...
export function parseDataEngineFactEnvelope(
  ...
  if (typeof candidate.engine_version !== 'string' || candidate.engine_version.length === 0) return null;
  if (candidate.source_owner !== DATA_ENGINE_SOURCE_OWNER) return null;
  if (!dataEngineJurisdictions.includes(candidate.jurisdiction as DataEngineJurisdiction)) return null;
  if (!dataEngineResourceKinds.includes(candidate.resource_kind as DataEngineResourceKind)) return null;
