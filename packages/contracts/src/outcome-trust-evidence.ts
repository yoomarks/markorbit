...
export function parseOutcomeEvidenceReferenceV1(value: unknown): OutcomeEvidenceReferenceV1 {
  ...
  if (
    ...
    !allowedOwners.includes(reference.sourceOwner as OutcomeEvidenceSourceOwnerV1)
  )
    invalid('outcomeEvidenceReference.sourceOwner is invalid.');
  if (
    typeof reference.authorityState !== 'string' ||
    !SOURCE_AUTHORITY_STATES.has(reference.authorityState)
  )
    invalid('outcomeEvidenceReference.authorityState is invalid.');
  const effectiveFrom = optionalInstant(
    reference.effectiveFrom,
    'outcomeEvidenceReference.effectiveFrom'
  );
  const effectiveUntil = optionalInstant(
    reference.effectiveUntil,
    'outcomeEvidenceReference.effectiveUntil'
  );
  ...
  return {
    evidenceReference: text(
      ...
      'outcomeEvidenceReference.evidenceReference',
      800
    ),
    ...
    sourceFingerprintSha256: sha256(
      ...
      'outcomeEvidenceReference.sourceFingerprintSha256'
    ),
    ...
    artifactAccessAuthorized: falseOnly(
      ...
      'outcomeEvidenceReference.artifactAccessAuthorized'
    ),
    currentArtifactAuthorizationRequired: trueOnly(
      reference.currentArtifactAuthorizationRequired,
      'outcomeEvidenceReference.currentArtifactAuthorizationRequired'
    )
  };
}
...
export function parseOutcomeObservationReferenceV1(value: unknown): OutcomeObservationReferenceV1 {
  ...
  if (
    typeof observation.observationOwner !== 'string' ||
    !OBSERVATION_OWNERS.has(observation.observationOwner)
  )
    invalid('outcomeObservation.observationOwner is invalid.');
  if (
    typeof observation.lifecycleState !== 'string' ||
    !LIFECYCLE_STATES.has(observation.lifecycleState)
  )
    invalid('outcomeObservation.lifecycleState is invalid.');
  const effectiveFrom = optionalInstant(
    observation.effectiveFrom,
    'outcomeObservation.effectiveFrom'
  );
  const effectiveUntil = optionalInstant(
    observation.effectiveUntil,
    'outcomeObservation.effectiveUntil'
  );
  if (effectiveFrom && effectiveUntil && effectiveUntil <= effectiveFrom)
    invalid('outcomeObservation effective period is inverted.');
  return {
    schemaVersion: 1,
    outcomeObservationId: prefixed<OutcomeObservationIdV1>(
      observation.outcomeObservationId,
      'outcome-observation_',
      'outcomeObservation.outcomeObservationId'
    ),
    observationOwner: observation.observationOwner as OutcomeObservationOwnerV1,
    observerReference: text(
      observation.observerReference,
      'outcomeObservation.observerReference',
      500
    ),
    observerAuthorityReference: text(
      observation.observerAuthorityReference,
      'outcomeObservation.observerAuthorityReference',
      500
    ),
    providerId: prefixed<ProviderId>(
      observation.providerId,
      'provider_',
      'outcomeObservation.providerId'
    ),
    contextFingerprintSha256: sha256(
      observation.contextFingerprintSha256,
      'outcomeObservation.contextFingerprintSha256'
    ),
    ...
    observationFingerprintSha256: sha256(
      ...
      'outcomeObservation.observationFingerprintSha256'
    ),
    ...
    evidenceReferences: parseEvidenceReferences(
      ...
      'outcomeObservation.evidenceReferences'
    ),
    publicReviewCreated: falseOnly(
      observation.publicReviewCreated,
      'outcomeObservation.publicReviewCreated'
    ),
    officialTruthCreated: falseOnly(
      observation.officialTruthCreated,
      'outcomeObservation.officialTruthCreated'
    )
  };
}
...
function parseSource(value: unknown): TrustEvidenceSourceReferenceV1 {
  ...
  if (source.kind === 'CANONICAL_OWNER_FACT') {
    ...
    if (source.owner !== ownerByKind[source.factKind])
      invalid('trustEvidenceSource.owner does not own the selected factKind.');
    const effectiveFrom = optionalInstant(
      source.effectiveFrom,
      'trustEvidenceSource.effectiveFrom'
    );
    const effectiveUntil = optionalInstant(
      source.effectiveUntil,
      'trustEvidenceSource.effectiveUntil'
    );
    if (effectiveFrom && effectiveUntil && effectiveUntil <= effectiveFrom)
    ...
    return {
      ...
      sourceFingerprintSha256: sha256(
        ...
        'trustEvidenceSource.sourceFingerprintSha256'
      ),
      recordedAt: instant(source.recordedAt, 'trustEvidenceSource.recordedAt'),
      ...(effectiveFrom ? { effectiveFrom } : {}),
      ...(effectiveUntil ? { effectiveUntil } : {}),
      performanceTruthEstablished: falseOnly(
        source.performanceTruthEstablished,
        'trustEvidenceSource.performanceTruthEstablished'
      ),
      officialTruthEstablished: falseOnly(
        source.officialTruthEstablished,
        'trustEvidenceSource.officialTruthEstablished'
      )
    };
  }
  if (source.kind === 'PROVIDER_CLAIM') {
    ...
    return {
      kind: 'PROVIDER_CLAIM',
      owner: 'MGSN',
      providerReturnId: prefixed<ProviderReturnId>(
        source.providerReturnId,
        'provider-return_',
        'trustEvidenceSource.providerReturnId'
      ),
      providerReturnVersion: positiveInteger(
        source.providerReturnVersion,
        'trustEvidenceSource.providerReturnVersion'
      ),
      providerReturnFingerprintSha256: sha256(
        source.providerReturnFingerprintSha256,
        'trustEvidenceSource.providerReturnFingerprintSha256'
      ),
      ...
      verifiedOutcomeEstablished: falseOnly(
        source.verifiedOutcomeEstablished,
        'trustEvidenceSource.verifiedOutcomeEstablished'
      ),
      officialTruthEstablished: falseOnly(
        source.officialTruthEstablished,
        'trustEvidenceSource.officialTruthEstablished'
  ...
  exactKeys(
    source,
    ['kind', 'observation', 'universalPerformanceTruthEstablished', 'officialTruthEstablished'],
    'trustEvidenceSource'
  );
  return {
    kind: 'AUTHORIZED_OUTCOME_OBSERVATION',
    observation: parseOutcomeObservationReferenceV1(source.observation),
    universalPerformanceTruthEstablished: falseOnly(
      source.universalPerformanceTruthEstablished,
      'trustEvidenceSource.universalPerformanceTruthEstablished'
    ),
    officialTruthEstablished: falseOnly(
      source.officialTruthEstablished,
      'trustEvidenceSource.officialTruthEstablished'
...
function parseSourceAuthority(
  value: unknown,
  sourceKind: TrustEvidenceSourceReferenceV1['kind']
): TrustEvidenceSourceAuthorityV1 {
  ...
  if (
    ...
    !SOURCE_AUTHORITY_STATES.has(authority.authorityState)
  )
  ...
  return {
    ...
    currentSourceRevalidationRequiredBeforeUse: trueOnly(
      authority.currentSourceRevalidationRequiredBeforeUse,
      'trustEvidenceSourceAuthority.currentSourceRevalidationRequiredBeforeUse'
    ),
    historicalSourceDoesNotEstablishCurrentSuitability: trueOnly(
      authority.historicalSourceDoesNotEstablishCurrentSuitability,
      'trustEvidenceSourceAuthority.historicalSourceDoesNotEstablishCurrentSuitability'
    ),
    universalPerformanceInferenceAuthorized: falseOnly(
      authority.universalPerformanceInferenceAuthorized,
      'trustEvidenceSourceAuthority.universalPerformanceInferenceAuthorized'
    )
  };
}
...
function parseProfileReference(value: unknown): ProviderResponsibilityProfileReferenceV1 {
  ...
  exactKeys(
    profile,
    ['providerResponsibilityProfileId', 'version', 'profileFingerprintSha256'],
    'executorAttribution.profile'
  );
  return {
    providerResponsibilityProfileId: prefixed(
      profile.providerResponsibilityProfileId,
      'provider-responsibility_',
      'executorAttribution.profile.providerResponsibilityProfileId'
    ),
    version: positiveInteger(profile.version, 'executorAttribution.profile.version'),
    profileFingerprintSha256: sha256(
      profile.profileFingerprintSha256,
      'executorAttribution.profile.profileFingerprintSha256'
    )
  };
}
...
function parseExecutorAttribution(value: unknown): TrustEvidenceExecutorAttributionV1 {
  ...
  if (executor.state === 'ESTABLISHED') {
    ...
    return {
      ...
      assessmentReference: text(
        executor.assessmentReference,
        'executorAttribution.assessmentReference',
        500
      ),
      assessmentFingerprintSha256: sha256(
        executor.assessmentFingerprintSha256,
        'executorAttribution.assessmentFingerprintSha256'
      ),
      profile: parseProfileReference(executor.profile),
      finalExecutionProviderId: prefixed<ProviderId>(
        executor.finalExecutionProviderId,
        'provider_',
        'executorAttribution.finalExecutionProviderId'
      ),
      checkedAt: instant(executor.checkedAt, 'executorAttribution.checkedAt'),
      currentAuthorityRevalidationRequiredBeforeUse: trueOnly(
        executor.currentAuthorityRevalidationRequiredBeforeUse,
        'executorAttribution.currentAuthorityRevalidationRequiredBeforeUse'
      )
    };
  }
  ...
  if (
    typeof executor.assessmentState !== 'string' ||
    POSITIVE_EXECUTOR_STATES.has(executor.assessmentState)
  )
  ...
  if (!allowedNonPositive.has(executor.assessmentState))
  ...
  const assessmentReference =
    executor.assessmentReference === undefined
      ? undefined
      : text(executor.assessmentReference, 'executorAttribution.assessmentReference', 500);
  const assessmentFingerprintSha256 =
    executor.assessmentFingerprintSha256 === undefined
      ? undefined
      : sha256(
          executor.assessmentFingerprintSha256,
          'executorAttribution.assessmentFingerprintSha256'
  ...
  return {
    state: 'NOT_ESTABLISHED',
    assessmentState: executor.assessmentState as Exclude<
      ProviderDirectExecutorAssessmentState,
      'DIRECT_FINAL_EXECUTOR_ESTABLISHED' | 'DIRECT_EXECUTOR_WITH_REQUIRED_SIGNER_ESTABLISHED'
    >,
    ...assessmentIdentity,
...
function parseContext(value: unknown): TrustEvidenceContextV1 {
  ...
  return {
    providerId: prefixed<ProviderId>(
      context.providerId,
      'provider_',
      'trustEvidenceContext.providerId'
    ),
    contextReference: text(context.contextReference, 'trustEvidenceContext.contextReference', 500),
    contextFingerprintSha256: sha256(
      context.contextFingerprintSha256,
      'trustEvidenceContext.contextFingerprintSha256'
    ),
    ...
    collaborationScope: text(
      ...
      'trustEvidenceContext.collaborationScope',
      500
    ),
    executorAttribution: parseExecutorAttribution(context.executorAttribution),
    clientIdentityEmbedded: falseOnly(
      context.clientIdentityEmbedded,
      'trustEvidenceContext.clientIdentityEmbedded'
    ),
    relationshipIdentityEmbedded: falseOnly(
      context.relationshipIdentityEmbedded,
      'trustEvidenceContext.relationshipIdentityEmbedded'
    ),
    commercialDataEmbedded: falseOnly(
      context.commercialDataEmbedded,
      'trustEvidenceContext.commercialDataEmbedded'
    )
  };
}
...
function parseFreshness(
  ...
  exactKeys(
    ...
    [
      ...
      'effectiveFrom',
      'effectiveUntil',
      'currentSuitabilityEstablished'
    ],
    'trustEvidenceFreshness'
  );
  ...
  const effectiveFrom = optionalInstant(
    freshness.effectiveFrom,
    'trustEvidenceFreshness.effectiveFrom'
  );
  const effectiveUntil = optionalInstant(
    freshness.effectiveUntil,
    'trustEvidenceFreshness.effectiveUntil'
  );
  if (effectiveFrom && effectiveUntil && effectiveUntil <= effectiveFrom)
  ...
  return {
    ...
    ...(effectiveFrom ? { effectiveFrom } : {}),
    ...(effectiveUntil ? { effectiveUntil } : {}),
    currentSuitabilityEstablished: falseOnly(
      freshness.currentSuitabilityEstablished,
      'trustEvidenceFreshness.currentSuitabilityEstablished'
    )
  };
}
...
function parseItemReference(
  value: unknown,
  field = 'trustEvidenceItemReference'
): TrustEvidenceItemReferenceV1 {
  const reference = record(value, field);
  exactKeys(
    reference,
    ['trustEvidenceItemId', 'version', 'trustEvidenceItemFingerprintSha256'],
    field
  );
  return {
    trustEvidenceItemId: prefixed<TrustEvidenceItemIdV1>(
      reference.trustEvidenceItemId,
      'trust-evidence-item_',
      `${field}.trustEvidenceItemId`
    ),
    ...
    trustEvidenceItemFingerprintSha256: sha256(
      ...
      `${field}.trustEvidenceItemFingerprintSha256`
    )
  };
}
...
function parseLineage(value: unknown): TrustEvidenceLineageReferenceV1 {
  ...
  exactKeys(
    lineage,
    ['trustEvidenceItemId', 'version', 'trustEvidenceItemFingerprintSha256', 'relation'],
    'trustEvidenceLineage'
  );
...
function parseContradictionReference(value: unknown): TrustEvidenceContradictionReferenceV1 {
  ...
  return {
    ...
    contradictionReference: text(
      contradiction.contradictionReference,
      'trustEvidenceContradiction.contradictionReference',
      500
    )
  };
}
...
export function trustEvidenceVisibilityProjectionFingerprintV1(
  value: TrustEvidenceProjectionFingerprintInputV1
): string {
  return fingerprint(value);
}
...
export function parseTrustEvidenceItemV1(value: unknown): TrustEvidenceItemV1 {
  ...
  const providerId = prefixed<ProviderId>(
    item.providerId,
    'provider_',
    'trustEvidenceItem.providerId'
  );
  const context = parseContext(item.context);
  if (context.providerId !== providerId)
    invalid('trustEvidenceItem context Provider must match item Provider.');
  ...
  if (
    item.lifecycleState === 'SUPERSEDED' &&
    source.kind === 'PROVIDER_CLAIM' &&
    source.providerReturnStatus !== 'SUPERSEDED'
  )
    invalid(
      'SUPERSEDED Provider Claim Trust Evidence must reference a SUPERSEDED Provider Return.'
    );
  if (
    !Array.isArray(item.lineage) ||
  ...
  const base: TrustEvidenceItemFingerprintInputV1 = {
    ...
    evidenceReferences: parseEvidenceReferences(
      ...
      'trustEvidenceItem.evidenceReferences'
    ),
    freshness,
    ...
    currentExposureAuthorizationRequired: trueOnly(
      item.currentExposureAuthorizationRequired,
      'trustEvidenceItem.currentExposureAuthorizationRequired'
    ),
    authorityConsequences: parseAuthority(item.authorityConsequences)
  };
  const trustEvidenceItemFingerprintSha256 = sha256(
    item.trustEvidenceItemFingerprintSha256,
    'trustEvidenceItem.trustEvidenceItemFingerprintSha256'
  );
  ...
  const trustEvidenceItemId = prefixed<TrustEvidenceItemIdV1>(
    item.trustEvidenceItemId,
    'trust-evidence-item_',
    'trustEvidenceItem.trustEvidenceItemId'
  );
...
function parseAudience(value: unknown): TrustEvidenceProjectionAudienceV1 {
  ...
  if (audience.kind === 'TRUSTED_RELATIONSHIP') {
    exactKeys(
      audience,
      ['kind', 'relationshipAuthorityReference'],
      'trustEvidenceProjection.audience'
    );
    return {
      kind: 'TRUSTED_RELATIONSHIP',
      relationshipAuthorityReference: text(
        audience.relationshipAuthorityReference,
        'trustEvidenceProjection.audience.relationshipAuthorityReference',
        500
      )
    };
  }
  if (audience.kind !== 'OWNER_WORKSPACE')
    invalid('trustEvidenceProjection.audience.kind is invalid.');
  exactKeys(audience, ['kind', 'ownerAuthorityReference'], 'trustEvidenceProjection.audience');
  return {
    kind: 'OWNER_WORKSPACE',
    ownerAuthorityReference: text(
      audience.ownerAuthorityReference,
      'trustEvidenceProjection.audience.ownerAuthorityReference',
      500
    )
  };
}

function parseHistoricalAuthorization(
  value: unknown
): TrustEvidenceVisibilityAuthorizationReferenceV1 {
  ...
  if (authorization.kind === 'NETWORK_VISIBILITY') {
    ...
    if (authorization.networkPurpose !== 'PROVIDER_DISCOVERY')
      invalid(
        'Network visibility authorization must preserve the existing PROVIDER_DISCOVERY purpose.'
      );
    return {
      kind: 'NETWORK_VISIBILITY',
      networkParticipationId: prefixed<NetworkParticipationId>(
        authorization.networkParticipationId,
        'network-participation_',
        'trustEvidenceProjection.historicalAuthorization.networkParticipationId'
      ),
      participationVersion: positiveInteger(
        authorization.participationVersion,
        'trustEvidenceProjection.historicalAuthorization.participationVersion'
      ),
      visibilityPolicyVersion: positiveInteger(
        authorization.visibilityPolicyVersion,
        'trustEvidenceProjection.historicalAuthorization.visibilityPolicyVersion'
      ),
      visibilityAuthorizationReference: text(
        authorization.visibilityAuthorizationReference,
        'trustEvidenceProjection.historicalAuthorization.visibilityAuthorizationReference',
        500
      ),
      networkPurpose: 'PROVIDER_DISCOVERY',
      trustProjectionAuthorizationReference: text(
        authorization.trustProjectionAuthorizationReference,
        'trustEvidenceProjection.historicalAuthorization.trustProjectionAuthorizationReference',
        500
      ),
      evaluatedAt: instant(
        authorization.evaluatedAt,
        'trustEvidenceProjection.historicalAuthorization.evaluatedAt'
      ),
      currentAuthorityRevalidationRequiredBeforeServe: trueOnly(
        authorization.currentAuthorityRevalidationRequiredBeforeServe,
        'trustEvidenceProjection.historicalAuthorization.currentAuthorityRevalidationRequiredBeforeServe'
      )
    };
  }
  ...
  return {
    ...
    authorityReference: text(
      ...
      'trustEvidenceProjection.historicalAuthorization.authorityReference',
      500
    ),
    authorityVersion: version(
      authorization.authorityVersion,
      'trustEvidenceProjection.historicalAuthorization.authorityVersion'
    ),
    authorityFingerprintSha256: sha256(
      authorization.authorityFingerprintSha256,
      'trustEvidenceProjection.historicalAuthorization.authorityFingerprintSha256'
    ),
    evaluatedAt: instant(
      authorization.evaluatedAt,
      'trustEvidenceProjection.historicalAuthorization.evaluatedAt'
...
export function parseTrustEvidenceVisibilityProjectionV1(
  value: unknown
): TrustEvidenceVisibilityProjectionV1 {
  ...
  if (projection.purpose === 'PROVIDER_DISCOVERY_TRUST_EXPLANATION') {
    if (audience.kind !== 'BOUNDED_NETWORK' && audience.kind !== 'TRUSTED_RELATIONSHIP')
      invalid(
        'Provider Discovery Trust explanation requires a network or trusted-relationship audience.'
      );
    if (historicalAuthorization.kind !== 'NETWORK_VISIBILITY')
      invalid(
        'Provider Discovery Trust explanation requires Network Visibility provenance plus separate Trust projection authorization.'
      );
  }
  const base: TrustEvidenceProjectionFingerprintInputV1 = {
    ...
    providerId: prefixed<ProviderId>(
      ...
      'provider_',
      'trustEvidenceProjection.providerId'
    ),
    purpose: projection.purpose as TrustEvidenceProjectionPurposeV1,
    audience,
    contextFingerprintSha256: sha256(
      projection.contextFingerprintSha256,
      'trustEvidenceProjection.contextFingerprintSha256'
    ),
    evidenceItems: projection.evidenceItems.map((entry) => parseItemReference(entry)),
    projectedFields,
    historicalAuthorization,
    artifactAccessAuthorized: falseOnly(
      projection.artifactAccessAuthorized,
      'trustEvidenceProjection.artifactAccessAuthorized'
    ),
    rawEvidenceDisclosureAuthorized: falseOnly(
      projection.rawEvidenceDisclosureAuthorized,
      'trustEvidenceProjection.rawEvidenceDisclosureAuthorized'
    ),
    relationshipGraphDisclosureAuthorized: falseOnly(
      projection.relationshipGraphDisclosureAuthorized,
      'trustEvidenceProjection.relationshipGraphDisclosureAuthorized'
    ),
    clientDataDisclosureAuthorized: falseOnly(
      projection.clientDataDisclosureAuthorized,
      'trustEvidenceProjection.clientDataDisclosureAuthorized'
    ),
    commercialDataDisclosureAuthorized: falseOnly(
      projection.commercialDataDisclosureAuthorized,
      'trustEvidenceProjection.commercialDataDisclosureAuthorized'
    ),
    currentAuthorityRevalidationRequiredBeforeServe: trueOnly(
      projection.currentAuthorityRevalidationRequiredBeforeServe,
      'trustEvidenceProjection.currentAuthorityRevalidationRequiredBeforeServe'
    ),
    authorityConsequences: parseAuthority(projection.authorityConsequences)
  };
  const projectionFingerprintSha256 = sha256(
    projection.projectionFingerprintSha256,
    'trustEvidenceProjection.projectionFingerprintSha256'
  );
  ...
  const trustEvidenceVisibilityProjectionId = prefixed<TrustEvidenceVisibilityProjectionIdV1>(
    projection.trustEvidenceVisibilityProjectionId,
    'trust-evidence-projection_',
    'trustEvidenceProjection.trustEvidenceVisibilityProjectionId'
  );
  if (
    trustEvidenceVisibilityProjectionId !==
    `trust-evidence-projection_${projectionFingerprintSha256}`
  )
...
export function parseTrustEvidenceCurrentExposureValidationV1(
  value: unknown
  ...
  if (validation.schemaVersion !== 1)
    invalid('trustEvidenceExposureValidation.schemaVersion must be 1.');
  ...
  exactKeys(
    ...
    'trustEvidenceExposureValidation.projection'
  );
  const projectionReference = {
    trustEvidenceVisibilityProjectionId: prefixed<TrustEvidenceVisibilityProjectionIdV1>(
      projection.trustEvidenceVisibilityProjectionId,
      'trust-evidence-projection_',
      'trustEvidenceExposureValidation.projection.trustEvidenceVisibilityProjectionId'
    ),
    projectionFingerprintSha256: sha256(
      projection.projectionFingerprintSha256,
      'trustEvidenceExposureValidation.projection.projectionFingerprintSha256'
    )
  };
  const base = {
    ...
    providerId: prefixed<ProviderId>(
      ...
      'provider_',
      'trustEvidenceExposureValidation.providerId'
    ),
    purpose: validation.purpose as TrustEvidenceProjectionPurposeV1,
    contextFingerprintSha256: sha256(
      validation.contextFingerprintSha256,
      'trustEvidenceExposureValidation.contextFingerprintSha256'
    ),
    projection: projectionReference,
    checkedAt: instant(validation.checkedAt, 'trustEvidenceExposureValidation.checkedAt'),
    artifactAccessAuthorized: falseOnly(
      validation.artifactAccessAuthorized,
      'trustEvidenceExposureValidation.artifactAccessAuthorized'
    ),
  ...
  if (validation.decision === 'AUTHORIZED_FOR_BOUNDED_TRUST_EXPLANATION') {
    exactKeys(
      validation,
      [...common, 'validatedEvidenceItems', 'authorityReferences'],
      'trustEvidenceExposureValidation'
    );
    if (
      !Array.isArray(validation.validatedEvidenceItems) ||
      !Array.isArray(validation.authorityReferences)
    )
      invalid(
        'Authorized Trust exposure validation requires evidenceItems and authorityReferences arrays.'
      );
    ...
    return {
      ...base,
      decision: 'AUTHORIZED_FOR_BOUNDED_TRUST_EXPLANATION',
      validatedEvidenceItems: validation.validatedEvidenceItems.map((entry) =>
        parseItemReference(entry)
      ),
      authorityReferences: validation.authorityReferences.map((entry, index) =>
        text(entry, `trustEvidenceExposureValidation.authorityReferences[${index}]`, 500)
      )
    };
  }
  if (validation.decision !== 'DENY')
    invalid('trustEvidenceExposureValidation.decision is invalid.');
...
export function parseTrustExplanationV1(value: unknown): TrustExplanationV1 {
  ...
  if (
    !Array.isArray(explanation.evidenceItems) ||
    !Array.isArray(explanation.contradictions) ||
  ...
  const contradictions = explanation.contradictions.map((entry) =>
    parseExplanationContradiction(entry)
  );
  ...
  if (explanation.result === 'CONTRADICTORY_EVIDENCE' && contradictions.length < 1)
    invalid(
      'CONTRADICTORY_EVIDENCE explanation requires explicit contradictory source references.'
    );
  const projection = record(
    explanation.visibilityProjection,
    'trustExplanation.visibilityProjection'
  );
  exactKeys(
    projection,
    ['trustEvidenceVisibilityProjectionId', 'projectionFingerprintSha256'],
  ...
  const base: TrustExplanationFingerprintInputV1 = {
    ...
    providerId: prefixed<ProviderId>(
      ...
      'provider_',
      'trustExplanation.providerId'
    ),
    contextFingerprintSha256: sha256(
      explanation.contextFingerprintSha256,
      'trustExplanation.contextFingerprintSha256'
    ),
    ...
    visibilityProjection: {
      trustEvidenceVisibilityProjectionId: prefixed<TrustEvidenceVisibilityProjectionIdV1>(
        projection.trustEvidenceVisibilityProjectionId,
        'trust-evidence-projection_',
        'trustExplanation.visibilityProjection.trustEvidenceVisibilityProjectionId'
      ),
      projectionFingerprintSha256: sha256(
        projection.projectionFingerprintSha256,
        'trustExplanation.visibilityProjection.projectionFingerprintSha256'
      )
    },
    currentExposureValidationRequiredBeforeServe: trueOnly(
      explanation.currentExposureValidationRequiredBeforeServe,
      'trustExplanation.currentExposureValidationRequiredBeforeServe'
    ),
    universalScoreCreated: falseOnly(
      explanation.universalScoreCreated,
      'trustExplanation.universalScoreCreated'
    ),
  ...
  const trustExplanationFingerprintSha256 = sha256(
    explanation.trustExplanationFingerprintSha256,
    'trustExplanation.trustExplanationFingerprintSha256'
  );
  ...
  const trustExplanationId = prefixed<TrustExplanationIdV1>(
    explanation.trustExplanationId,
    'trust-explanation_',
    'trustExplanation.trustExplanationId'
  );
...
export const outcomeTrustEvidenceFixtureProviderIdV1 =
  'provider_fixture-trust-439' as const satisfies ProviderId;
...
export const outcomeTrustEvidenceFixtureCorrelationIdV1 =
  'correlation_outcome_trust_439' as const satisfies MarkOrbitId;
