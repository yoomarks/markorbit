...
export type TrustedPublicServeDenialReasonV1 =
  (typeof trustedPublicServeDenialReasons)[number];
...
function hasNoAuthorityConsequences(value: unknown): value is TrustedPublicAuthorityConsequencesV1 {
  ...
  return Object.keys(noTrustedPublicAuthorityConsequences).every(
...
export function parseTrustedPublicEligibilityAuthorizationV1(
  ...
  if (!item.subjectProviderId || !item.subjectOrganizationReference) throw new Error('Invalid subject');
  if (!item.authorizingPrincipalReference || !item.authorizationOwnerReference) throw new Error('Invalid authority');
  if (!item.purpose || !trustedPublicAudienceKinds.includes(item.audience)) throw new Error('Invalid purpose/audience');
  if (!Array.isArray(item.allowedFields) || item.allowedFields.length === 0) throw new Error('Empty public allowlist');
  if (new Set(item.allowedFields).size !== item.allowedFields.length) throw new Error('Duplicate public field');
  if (item.allowedFields.some((field) => !trustedPublicFieldClasses.includes(field))) throw new Error('Invalid public field');
  ...
  if (item.currentAuthorityRevalidationRequiredBeforeServe !== true ||
  ...
  const { eligibilityFingerprintSha256, ...body } = item;
  const expected = trustedPublicEligibilityFingerprintV1(body);
  if (eligibilityFingerprintSha256 !== expected || item.eligibilityId !== trustedPublicEligibilityIdV1(expected)) {
...
export function parseTrustedPublicProjectionV1(value: unknown): TrustedPublicProjectionV1 {
  ...
  if (!trustedPublicAudienceKinds.includes(item.audience) || !Array.isArray(item.fields) || item.fields.length === 0) {
  ...
  if (item.currentAuthorityRevalidationRequiredBeforeServe !== true || item.rawEvidenceEmbedded !== false ||
      item.artifactRetrievalAuthorized !== false || item.clientIdentityEmbedded !== false ||
      item.relationshipGraphEmbedded !== false || item.commercialDataEmbedded !== false ||
      item.internalWorkspaceIdentityEmbedded !== false || !hasNoAuthorityConsequences(item.authorityConsequences)) {
    throw new Error('Invalid projection privacy/authority locks');
  }
  const { projectionFingerprintSha256, ...body } = item;
  const expected = trustedPublicProjectionFingerprintV1(body);
  if (projectionFingerprintSha256 !== expected || item.projectionId !== trustedPublicProjectionIdV1(expected)) {
...
export function parseTrustedPublicServeDecisionV1(value: unknown): TrustedPublicServeDecisionV1 {
  ...
  if (item.schemaVersion !== 1 || item.currentAuthorityRevalidationPerformed !== true ||
      !hasNoAuthorityConsequences(item.authorityConsequences)) {
    throw new Error('Invalid serve authority locks');
  }
  assertIso(item.checkedAt, 'checkedAt');
  if (item.decision === 'AUTHORIZED') {
    if (item.state !== 'PUBLICLY_EXPOSED' || !item.projectionId || !item.eligibilityId ||
        !isHexSha256(item.projectionFingerprintSha256) || !isHexSha256(item.eligibilityFingerprintSha256)) {
      throw new Error('Invalid authorized public serve');
    }
  } else if (item.decision === 'DENY') {
    if (item.state === 'PUBLICLY_EXPOSED' || !trustedPublicServeDenialReasons.includes(item.reason)) {
