...
describe('ReadyPackage Content Export consumer contract', () => {
  ...
  it.each(['STANDARD_SOURCE', 'GLOBAL_REFERENCE'] as const)(
    'accepts governed V1.1 %s exports and preserves governance in exact serialization',
    async (kind) => {
      ...
      const serialized = serializeReadyPackageContentExportV1(parsed!);
  ...
  it('rejects a tagged governance payload with an unknown policy value', async () => {
    const value = await governedFixture('GLOBAL_REFERENCE');
    const governance = value.sourceGovernance as Record<string, unknown>;
    governance.contentReusePolicy = 'COPY_VERBATIM';
    expect(parseReadyPackageContentExportV1(value)).toBe(null);
  });
