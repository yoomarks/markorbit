...
describe('MGSN Provider Discovery Candidate V1 shared contract', () => {
  it(
    'freezes candidate-only authority with no Selection, Allocation, Acceptance or protected action consequence',
    () => {
      expect(
        Object.values(noProviderDiscoveryAuthorityConsequences).every((value) => !value)
      ).toBe(true);
      ...
      const candidate =
        providerDiscoveryContractFixtureV1.candidateResult.candidates[0];
      expect(candidate.authorityConsequences).toEqual(
        noProviderDiscoveryAuthorityConsequences
      );
    }
  );
  ...
  it(
    'uses an explicit authorized projection rather than Provider/Supply wholesale serialization',
    () => {
      const candidate =
        providerDiscoveryContractFixtureV1.candidateResult.candidates[0];
  ...
  it(
    'keeps visibility evidence, suitability evidence and source authority/version/freshness explicit',
    () => {
      ...
      expect(
        candidate.sourceVersions.every((source) => source.version !== undefined)
      ).toBe(true);
      expect(
        candidate.sourceVersions.every((source) => source.authorityState === 'CURRENT')
      ).toBe(true);
  ...
  it(
    'requires current visibility revalidation instead of treating historical candidate replay as permission',
    () => {
      const candidate =
        providerDiscoveryContractFixtureV1.candidateResult.candidates[0];
  ...
  it(
    'does not assume Direct-to-Executor proof while #375 remains an independent evidence boundary',
    () => {
  ...
  it(
    'represents a privacy-safe fail-closed empty result without hidden Provider leakage',
    () => {
      ...
      const authorityUnavailable =
        providerDiscoveryContractFixtureV1.authorityUnavailableResult;
  ...
  it('defines no universal rank, winner or score semantics', () => {
    ...
    expect(providerDiscoveryContractFixtureV1.candidateResult).not.toHaveProperty(
      'winner'
    );
  });
});
