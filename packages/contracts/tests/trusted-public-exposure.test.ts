...
// Historical eligibility and projections never replace current-authority revalidation at serve time.
describe('Trusted Public Exposure V1', () => {
  ...
  it('fails closed for denied or malformed public serve state', () => {
    ...
    expect(() =>
      parseTrustedPublicServeDecisionV1({ ...denied, state: 'PUBLICLY_EXPOSED' })
    ).toThrow('Invalid denied public serve');
  });
});
