...
for (const row of inventory) {
  ...
  const authenticated =
    ...
    row.path.startsWith('/api/markreg/production-recommendations') ||
    row.path.startsWith('/api/markreg/production-user-selections') ||
    row.path.startsWith('/api/markreg/production-quotes') ||
    row.path.startsWith('/api/markreg/recommended-actions') ||
    row.path.startsWith('/api/markreg/workspace-actions') ||
    row.path.startsWith('/api/site/markreg/') ||
...
assert.equal(source.length, 106);
...
console.log(
  'Gateway inventory PASS: 106 runtime routes; authenticated Site admission, Early Funnel, Production Intake + Fee Facts, Production Recommendation, User Selection, Production Quote, Matter Intelligence, Formal Matter Evidence, Examination, Workspace Action, Checkout, Commercial Catalog, Payment, Order, Document Package, Evidence Review and Lifecycle boundaries included; test bootstrap excluded'
);
