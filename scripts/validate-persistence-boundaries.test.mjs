...
async function fixture() {
  ...
  await writeFile(
    ...
    '{"namespaces":{"alpha":"@markorbit/alpha-service","beta":"@markorbit/beta-service"},"protectedTableFamilies":{"capability_communication_":"@markorbit/capability-engine","lite_communication_link_":"@markorbit/lite-service","lite_work_item":"@markorbit/lite-service"},"migrations":{}}'
  );
  return root;
}
...
test('rejects a protected Agency truth table family under the wrong migration owner', async () => {
  const root = await fixture();
  await writeFile(
    path.join(root, 'infrastructure/persistence/migration-owners.json'),
    '{"namespaces":{"beta":"@markorbit/beta-service"},"protectedTableFamilies":{"capability_communication_":"@markorbit/capability-engine"},"migrations":{"9999_beta_shadow":"@markorbit/beta-service"}}'
  );
  await writeFile(
    path.join(root, 'infrastructure/persistence/migrations/9999_beta_shadow.sql'),
    'CREATE TABLE capability_communication_shadow (id text PRIMARY KEY);'
  );
  const failures = await persistenceBoundaryFailures(root);
  assert.ok(
    failures.some((value) =>
      value.includes(
        'may not create protected capability_communication_* table capability_communication_shadow'
      )
    )
  );
});
