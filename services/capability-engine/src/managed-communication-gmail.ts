...
type GmailProviderConfig = Readonly<{
  clientId: string;
  clientSecret: string;
  refreshToken: string;
...
function splitMailboxList(value: string | undefined): readonly Readonly<{ address: string; displayName?: string }>[] {
  if (!value) return [];
  return value
    .split(/,(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)/u)
    .map((item) => item.trim())
...
function participantList(payload: GmailPart): readonly Readonly<ManagedCommunicationParticipantV1>[] {
  ...
  const push = (
    ...
    for (const item of splitMailboxList(value)) {
      participants.push({ role, address: item.address, ...(item.displayName ? { displayName: item.displayName } : {}) });
    }
  };
...
function mimeMessage(
  ...
  if (request.attachments.length > 0)
    throw new Error('Gmail pilot sender does not accept outbound attachments without a governed byte resolver.');
  ...
  const to = byRole('TO');
  if (!sender || to.length === 0) throw new Error('Gmail sender requires sender and recipient participants.');
  const cc = byRole('CC');
  ...
  const headers = [
    ...
    ...(replyHeaders?.inReplyTo ? [`In-Reply-To: ${safeHeader(replyHeaders.inReplyTo, 'In-Reply-To')}`] : []),
    ...(replyHeaders?.references ? [`References: ${safeHeader(replyHeaders.references, 'References')}`] : []),
    'MIME-Version: 1.0'
  ];
  if (request.textBody && request.htmlBody) {
    const boundary = `markorbit_${sendId.replace(/[^a-zA-Z0-9_-]/gu, '_')}`;
    return [
      ...headers,
  ...
  return [
    ...
    'Content-Transfer-Encoding: 8bit',
    '',
    html ? request.htmlBody! : request.textBody!,
    ''
  ].join('\r\n');
}
...
export class GmailManagedCommunicationClientV1 {
  ...
  async profile(): Promise<Readonly<{ historyId: string }>> {
    ...
    return Object.freeze({ historyId: required((value as { historyId?: unknown }).historyId, 'gmail.profile.historyId', 200) });
  }
  ...
  async sendRaw(input: Readonly<{ raw: string; threadId?: string }>): Promise<GmailMessage> {
    return this.gmailJson('/users/me/messages/send', {
      ...
      body: JSON.stringify({ raw: Buffer.from(input.raw, 'utf8').toString('base64url'), ...(input.threadId ? { threadId: input.threadId } : {}) })
    }) as Promise<GmailMessage>;
  }

  async threadMetadata(threadId: string): Promise<Readonly<{ messageId?: string; references?: string; subject?: string }>> {
    const value = (await this.gmailJson(
      `/users/me/threads/${encodeURIComponent(threadId)}?format=metadata&metadataHeaders=Message-ID&metadataHeaders=References&metadataHeaders=Subject`
    )) as Readonly<{ messages?: readonly GmailMessage[] }>;
    const latest = value.messages?.at(-1)?.payload;
    return Object.freeze({
      ...(messageHeader(latest?.headers, 'Message-ID') ? { messageId: messageHeader(latest?.headers, 'Message-ID') } : {}),
      ...(messageHeader(latest?.headers, 'References') ? { references: messageHeader(latest?.headers, 'References') } : {}),
      ...(messageHeader(latest?.headers, 'Subject') ? { subject: messageHeader(latest?.headers, 'Subject') } : {})
    });
  }
  ...
  async message(id: string, format: 'full' | 'raw'): Promise<GmailMessage> {
    return this.gmailJson(`/users/me/messages/${encodeURIComponent(id)}?format=${format}`) as Promise<GmailMessage>;
  }
  ...
  private async token(): Promise<string> {
    ...
    const value = (await response.json()) as Readonly<{ access_token?: unknown; expires_in?: unknown }>;
...
export class GmailManagedCommunicationSenderV1 implements ManagedCommunicationProviderSenderV1 {
  constructor(
    ...
    private readonly resolveProviderThreadId?: (input: Readonly<{ workspaceId: string; accountRef: string; threadRef: string }>) => Promise<string | undefined>,
  ...
  async send(
    ...
    context: Readonly<{
      sendId: string;
      workspaceId: string;
      account: Readonly<{ provider: string; providerAccountRef: string; accountRef: string }>;
      correlationId: string;
    }>
  ) {
    if (context.account.provider !== GMAIL_PROVIDER)
      throw new Error('Gmail sender received a non-Gmail account binding.');
    if (context.account.providerAccountRef.toLowerCase() !== this.client.providerAccountRef().toLowerCase())
      throw new Error('Gmail sender account binding does not match the configured provider account.');

    let providerThreadId: string | undefined;
    let replyHeaders: Readonly<{ inReplyTo?: string; references?: string; subject?: string }> | undefined;
    if (request.replyToThreadRef) {
      ...
      if (!providerThreadId) throw new Error('Gmail reply thread could not be resolved durably.');
    ...
    const result = await this.client.sendRaw({
      raw: mimeMessage(request, context.sendId, replyHeaders),
...
export class GmailManagedCommunicationInboundV1 {
  ...
  async syncOnce(): Promise<GmailManagedCommunicationInboundResultV1> {
    ...
    const account = await this.options.foundation.resolveAccount(this.options.workspaceId, this.options.accountRef);
    if (account.provider !== GMAIL_PROVIDER) throw new Error('Gmail inbound sync received a non-Gmail account binding.');
    if (account.providerAccountRef.toLowerCase() !== this.options.client.providerAccountRef().toLowerCase())
      throw new Error('Gmail inbound account binding does not match the configured provider account.');

    const checkpoint = await this.options.foundation.latestCheckpoint(this.options.workspaceId, this.options.accountRef);
    if (!checkpoint) {
    ...
    do {
      ...
      if (page.historyId) providerCursor = required(page.historyId, 'gmail.history.historyId', 200);
      for (const history of page.history ?? []) {
        for (const added of history.messagesAdded ?? []) {
          if (added.message?.id) messageIds.add(required(added.message.id, 'gmail.history.message.id', 500));
        }
      }
  ...
  private async importInbound(messageId: string, now: () => string): Promise<number> {
    ...
    if (sender.address.toLowerCase() === this.options.client.providerAccountRef().toLowerCase()) return 0;
    ...
    const evidenceHeaders = (payload.headers ?? [])
      .filter((header) => header.name && header.value && !SENSITIVE_EVIDENCE_HEADER.test(header.name))
  ...
  private async attachments(messageId: string, payload: GmailPart): Promise<readonly Readonly<ManagedCommunicationAttachmentRefV1>[]> {
    ...
    for (const part of attachmentParts(payload)) {
      ...
      values.push({
        attachmentRef: attachmentId ? `gmail:${messageId}:${attachmentId}` : `gmail:${messageId}:part:${part.partId ?? values.length}`,
...
export class GmailManagedCommunicationPollerV1 {
  ...
  constructor(
    ...
    private readonly onError: () => void = () => {
      process.stderr.write('capability-engine: Managed Communication Gmail inbound sync failed; operator action may be required.\n');
    }
  ) {
  ...
  private async runOnce(): Promise<GmailManagedCommunicationInboundResultV1> {
    if (!this.running) this.running = this.inbound.syncOnce().finally(() => (this.running = undefined));
    return this.running;
  }
}
