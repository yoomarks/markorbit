...
function service(sourceAdmissionPolicies?: Readonly<CurrentSourceAdmissionPolicyReadV1>) {
...
function pilotPolicy(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    schemaVersion: 1,
...
describe('Capability cognitive read source-policy content identity', () => {
  it('projects exact current policy fingerprints', async () => {
    ...
    const expected = new Map(
      currentCapabilitySourceAdmissionPolicyCatalogV1
        ...
        .map((entry) => [
          entry.policyId,
          materializeCapabilitySourceAdmissionPolicyContentIdentityV1(entry).policyFingerprintSha256
        ])
    );
  ...
  it('preserves caller-order canonicalization', async () => {
    const first = await service({
      list: () => [pilotPolicy({ allowedCallerProducts: ['MARKREG', 'LITE'] })]
    }).read();
    const reordered = await service({
      list: () => [pilotPolicy({ allowedCallerProducts: ['LITE', 'MARKREG'] })]
    }).read();

    expect(first.sourceAdmissionPolicies[0]?.allowedCallerProducts).toEqual(['LITE', 'MARKREG']);
