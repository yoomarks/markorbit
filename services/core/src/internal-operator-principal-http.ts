  ...
  return [
    ...
    },
    {
      ...
      async handle(request) {
        ...
        const token = exactAdminResolutionRequest(request, 'Execution Admin');
        try {
          return json(200, await options.resolver.resolve(token, 'execution-admin:read'));
        } catch (error) {
          return translate(error);
        }
      }
    ...
    },
    {
      ...
      async handle(request) {
        ...
        const token = exactAdminResolutionRequest(request, 'Governance Admin');
        try {
          return json(200, await options.resolver.resolve(token, 'governance-admin:read'));
        } catch (error) {
          return translate(error);
        }
      }
