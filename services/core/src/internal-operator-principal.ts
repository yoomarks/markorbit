...
export class StaticCognitiveReadGrantSourceV1 implements CognitiveReadGrantSourceV1 {
  ...
  constructor(userIds: Iterable<string>) {
    ...
    if (normalized.some((userId) => !UUID.test(userId)))
    ...
    this.userIds = new Set(normalized);
  }

  async hasGrant(userId: string): Promise<boolean> {
    return this.userIds.has(userId.trim().toLowerCase());
  }
}
...
class UnavailableCognitiveReadGrantSourceV1 implements CognitiveReadGrantSourceV1 {
  ...
  async hasGrant(): Promise<boolean> {
    throw new CognitiveReadGrantSourceError('Cognitive read grant source is unavailable.', {
      cause: this.cause
    });
  }
}
...
type GrantConfig = {
  ...
  grants: readonly {
    userId: string;
    capabilities: readonly [typeof COGNITIVE_READ_CAPABILITY];
  }[];
};

function parseGrantConfig(value: string): GrantConfig {
  let parsed: unknown;
  try {
    parsed = JSON.parse(value);
  } catch (error) {
    throw new CognitiveReadGrantSourceError('Cognitive read grant configuration is malformed.', {
  ...
  const grants = config.grants.map((raw) => {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
      throw new CognitiveReadGrantSourceError('Cognitive read grant entry is malformed.');
    ...
    if (
      ...
      grant.capabilities[0] !== COGNITIVE_READ_CAPABILITY
    )
      throw new CognitiveReadGrantSourceError('Cognitive read grant entry is malformed.');
    return {
      userId: grant.userId.trim().toLowerCase(),
      capabilities: [COGNITIVE_READ_CAPABILITY] as const
    };
  });
...
export function createEnvironmentCognitiveReadGrantSourceV1(
  ...
  try {
    const config = parseGrantConfig(value);
...
export class InternalOperatorPrincipalResolverV1 {
  ...
  async resolve(token: string): Promise<Readonly<InternalOperatorPrincipal>> {
    ...
    let granted: boolean;
    try {
      granted = await this.options.cognitiveReadGrants.hasGrant(session.userId);
    } catch (error) {
      if (error instanceof CognitiveReadGrantSourceError)
        throw new AuthenticationError(
    ...
    if (!granted)
      throw new AuthenticationError(
        'PERMISSION_DENIED',
        'Explicit cognitive read grant is required.'
      );
    ...
    return Object.freeze({
      ...
      capabilities: Object.freeze([COGNITIVE_READ_CAPABILITY]),
