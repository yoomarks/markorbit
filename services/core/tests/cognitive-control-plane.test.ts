...
const proof = Object.freeze({
  kind: 'CAPABILITY_SOURCE_ADMISSION_EVIDENCE_V5',
  id: 'capability-source-admission-evidence_exact',
...
const cases: ReadonlyArray<
  readonly [CognitiveControlPlaneStateV1, Partial<CognitiveControlPlaneInputV1>]
> = [
  ['READY', {}],
...
describe('projectCognitiveControlPlaneV1', () => {
  ...
  it('fails closed on stale or ambiguous governed inputs before READY', () => {
  ...
  it('fails closed on missing, malformed or conflicting proof identity', () => {
    expect(projectCognitiveControlPlaneV1(base({ proofs: [] })).state).toBe('AUDIT_UNAVAILABLE');
    expect(
      projectCognitiveControlPlaneV1(
        base({ proofs: [{ ...proof, fingerprintSha256: 'not-a-sha256' }] })
      ).state
    ).toBe('AUDIT_UNAVAILABLE');
    expect(
      projectCognitiveControlPlaneV1(
        base({
          proofs: [proof, { ...proof, fingerprintSha256: 'b'.repeat(64) }]
        })
      ).state
    ).toBe('AUDIT_UNAVAILABLE');
  });
  ...
  it('prioritizes already-governed improvement state over ordinary readiness blockers', () => {
    expect(
      projectCognitiveControlPlaneV1(
  ...
  it('does not reinterpret READY as downstream execution authority', () => {
    ...
    expect(ready.authority.productStateCreated).toBe(false);
    ...
    expect(ready.authority.runtimeCapabilityCreated).toBe(false);
    expect(ready.authority.implementationApproved).toBe(false);
    expect(ready.authority.methodActivated).toBe(false);
  });
});
