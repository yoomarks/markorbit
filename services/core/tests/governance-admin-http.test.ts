...
import {
  createGovernanceAdminRoutesV1,
  type GovernanceAdministrationProjectionV1
} from '../src/governance-admin-http.js';

const secret = 'governance-admin-secret';
...
function route() {
  const found = createGovernanceAdminRoutesV1({
    internalServiceSecret: secret,
    now: () => new Date('2026-09-08T09:00:00.000Z')
  })[0];
  if (!found) throw new Error('Missing Governance Admin route.');
  return found;
}

describe('Governance owner administration read', () => {
  it('returns an explicit owner limitation instead of synthesizing a portfolio', async () => {
    ...
    const body = result.body as GovernanceAdministrationProjectionV1;
    expect(body).toEqual({
      schemaVersion: 1,
    ...
    expect(body.portfolio.reason).toContain(
      'canonical durable cross-owner Governance and Audit administration portfolio'
    );
  });

  it('rejects missing service authority and non-Governance operator authority', () => {
    expect(() =>
    ...
    ).toThrowError('Exact governance-admin:read authority is required.');
  });
});
