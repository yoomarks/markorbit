...
export class PostgresTrademarkServiceExecutionRepository {
  ...
  async createAuthorization(authorization: TrademarkServiceExecutionAuthorization) {
    try {
      await this.query.query(
        ...
        [authorization.workspaceId, authorization.executionAuthorizationId, authorization.workPackage.id,
          authorization.workPackage.version, authorization.executionReadinessId, JSON.stringify(authorization),
          authorization.authorizedByUserId, authorization.authorizedAt]
      );
    ...
    return this.database.transact(async (client) => {
      ...
      if (existing.rowCount) {
        ...
        if (String(row.request_fingerprint_sha256) !== release.requestFingerprintSha256)
          throw new TrademarkServiceExecutionError('IDEMPOTENCY_CONFLICT', 'Idempotency key was already used for a different protected action.');
      ...
      if (String(row.work_package_id) !== release.workPackage.id || Number(row.work_package_version) !== release.workPackage.version)
        throw new TrademarkServiceExecutionError('READINESS_REQUIRED', 'Protected action release does not match the frozen Work Package version.');
      await client.query(
        ...
        [release.workspaceId, release.idempotencyKey, release.requestFingerprintSha256,
          release.executionAuthorizationId, release.protectedActionReleaseId, JSON.stringify(release), release.releasedAt]
      );
      return release;
    });
  }

  async appendProviderHandoff(authorizationId: string, handoff: TrademarkServiceProviderHandoffRequest) {
    return this.appendArtifact(handoff.workspaceId, authorizationId, handoff.providerHandoffId, 'PROVIDER_HANDOFF', handoff);
  }
  async appendLifecycleHandoff(authorizationId: string, handoff: TrademarkServiceLifecycleHandoffRequest) {
    return this.appendArtifact(handoff.workspaceId, authorizationId, handoff.lifecycleHandoffId, 'LIFECYCLE_HANDOFF', handoff);
  }
  async appendEvidence(authorizationId: string, evidence: TrademarkServiceExecutionEvidence) {
    return this.appendArtifact(evidence.workspaceId, authorizationId, evidence.executionEvidenceId, 'EVIDENCE', evidence);
  }

  async saveRecovery(workspaceId: string, authorizationId: string, recovery: TrademarkServiceRecoveryState) {
    const result = await this.query.query(
    ...
    if (!result.rowCount) throw this.notFound();
    await this.appendArtifact(workspaceId, authorizationId, recoveryId(authorizationId, recovery), 'RECOVERY', recovery);
    return recovery;
  }

  async getSnapshot(workspaceId: string, authorizationId: string): Promise<TrademarkServiceExecutionSessionSnapshot | undefined> {
    const session = await this.query.query(
      `SELECT authorization_record,plan_record,recovery_record FROM execution_trademark_service_sessions
        WHERE workspace_id=$1 AND execution_authorization_id=$2`, [workspaceId, authorizationId]
    );
    ...
    const replays = await this.query.query(
      `SELECT release_record FROM execution_trademark_service_protected_action_replays
        WHERE workspace_id=$1 AND execution_authorization_id=$2 ORDER BY created_at`, [workspaceId, authorizationId]
    );
    const artifacts = await this.query.query(
      `SELECT artifact_kind,artifact_record FROM execution_trademark_service_artifacts
        WHERE workspace_id=$1 AND execution_authorization_id=$2 ORDER BY created_at,artifact_id`, [workspaceId, authorizationId]
    );
    const byKind = (kind: string) => artifacts.rows
    ...
    return {
      ...
      ...(row.recovery_record ? { recovery: row.recovery_record as TrademarkServiceRecoveryState } : {}),
      releases: replays.rows.map((item) => (item as Row).release_record as TrademarkServiceProtectedActionRelease),
  ...
  private async appendArtifact(workspaceId: string, authorizationId: string, artifactId: string,
    kind: 'PROVIDER_HANDOFF' | 'LIFECYCLE_HANDOFF' | 'EVIDENCE' | 'RECOVERY', record: unknown) {
    await this.query.query(
  ...
  private ensureWorkspace(expected: string, actual: string) {
    if (expected.toLowerCase() !== actual.toLowerCase())
      throw new TrademarkServiceExecutionError('WORKSPACE_MISMATCH', 'Execution record belongs to another Workspace.', 404);
  }
  private notFound() {
    return new TrademarkServiceExecutionError('OWNER_MISMATCH', 'Execution authorization was not found in this Workspace.', 404);
  }
  private unavailable(cause: unknown) {
    return new TrademarkServiceExecutionError('OWNER_MISMATCH', `Execution persistence is unavailable: ${cause instanceof Error ? cause.message : 'unknown error'}`, 503);
  }
}
