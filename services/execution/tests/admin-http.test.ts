...
import {
  createExecutionAdminRoutesV1,
  type ExecutionAdministrationProjectionV1
...
function route() {
  const found = createExecutionAdminRoutesV1({
    internalServiceSecret: secret,
    now: () => new Date('2026-09-08T09:00:00.000Z')
  })[0];
  if (!found) throw new Error('Missing Execution Admin route.');
  return found;
}

describe('Execution owner administration read', () => {
  it('returns an explicit owner limitation instead of synthesizing a portfolio', async () => {
    ...
    const body = result.body as ExecutionAdministrationProjectionV1;
    expect(body).toEqual({
      schemaVersion: 1,
  ...
  it('rejects missing service authority and non-Execution operator authority', () => {
    expect(() =>
    ...
    ).toThrowError('Exact execution-admin:read authority is required.');
  });
});
