...
function exactPurpose(formalMatterId: FormalMatterId): string {
  return (
    'Record descriptive completed-duration historical band for MarkReg ' +
    `Formal Matter ${formalMatterId}.`
  );
}
...
function assertExecution(
  ...
  if (
    ...
    throw new MatterIntelligenceError(
      'CAPABILITY_CONTRACT_MISMATCH',
      'Capability execution request does not match the exact MarkReg command.'
    );
  }
  ...
  if (
    ...
    throw new MatterIntelligenceError(
      'CAPABILITY_CONTRACT_MISMATCH',
      'Capability receipt or binding does not match the accepted identity.'
    );
  }
  ...
  if (
    ...
    throw new MatterIntelligenceError(
      'CAPABILITY_CONTRACT_MISMATCH',
      'Capability evidence is not bound to the accepted CN duration dataset.'
    );
  }
...
export class HttpCnDurationBandCapabilityClient implements MatterIntelligenceCapabilityClient {
  ...
  async classifyCompletedDuration(
    input: Readonly<{
      workspaceId: string;
      formalMatterId: FormalMatterId;
      observedCompletedDurationDays: number;
    ...
    try {
      response = await this.fetcher(
        `${this.capabilityUrl.replace(/\/$/, '')}/v1/capability-requests`,
        {
          method: 'POST',
          headers: {
    ...
    if (!response.ok) {
      ...
      throw new MatterIntelligenceError(
        unavailable ? 'CAPABILITY_UNAVAILABLE' : 'CAPABILITY_REJECTED',
        unavailable
          ? 'Governed Capability Runtime is unavailable.'
        ...
        async (client) => {
          ...
          const duplicate = await client.query(
            ...
            [
              value.observation.workspaceId,
              value.observation.capabilityReturnId,
              value.observation.sessionReceiptId
            ]
          );
          ...
          if (duplicate.rowCount) {
            ...
            if (
              ...
              throw new MatterIntelligenceError(
                'CAPABILITY_CONTRACT_MISMATCH',
                'Persisted Capability identity conflicts with this Formal Matter.'
              );
            }
