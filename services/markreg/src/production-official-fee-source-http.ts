...
export interface ProductionOfficialFeeSourceHttpOptionsV1 {
  readonly internalServiceSecret: string;
  readonly service: Pick<ProductionOfficialFeeSourceServiceV1, 'resolve'>;
}
...
function commandFor(request: JsonRequest): ResolveProductionOfficialFeeSourceCommandV1 {
  ...
  return {
    ...
    intakeId: text(body.intakeId) as ResolveProductionOfficialFeeSourceCommandV1['intakeId'],
    expectedIntakeVersion: Number(body.expectedIntakeVersion),
    idempotencyKey,
    correlationId: text(
      body.correlationId
    ) as ResolveProductionOfficialFeeSourceCommandV1['correlationId']
  };
}

function translate(error: unknown): never {
  if (error instanceof ProductionOfficialFeeSourceError) {
    throw new HttpError(error.status, error.code, error.message, error.retryable);
  }
  throw error;
}

export function createProductionOfficialFeeSourceRoutesV1(
  options: ProductionOfficialFeeSourceHttpOptionsV1
): readonly JsonRoute[] {
  return [
    {
      method: 'POST',
      path: '/internal/v1/production-official-fee-source/resolve',
      async handle(request) {
        const principal = principalFor(request, options.internalServiceSecret);
        try {
          return json(200, {
            officialFeeSource: await options.service.resolve(principal, commandFor(request))
          });
        } catch (error) {
