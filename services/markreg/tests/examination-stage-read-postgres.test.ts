...
suite('PostgreSQL Examination Stage V1 read projection', () => {
  ...
  function admission(suffix: string, formalMatterId: FormalMatterId): ReviewedSourceAdmissionEnvelope {
  ...
  it('reconstructs the exact Examination projection after read-side repository restart with no Examination persistence', async () => {
    ...
    const lifecycleRepository = new PostgresLifecycleProjectionRepository(database, database.getPool());
