...
if (required && !url)
  throw new Error(
    'MARKREG_TEST_DATABASE_URL is required when MARKREG_POSTGRES_TEST_REQUIRED=1.'
  );
...
suite('PostgreSQL Examination Stage V1 read projection', () => {
  ...
  it(
    'reconstructs the exact Examination projection after read-side repository restart with no Examination persistence',
    async () => {
      ...
      const projected = await lifecycle.project({
        ...
        customerSafeSummary:
          'Reviewed evidence is available for internal Examination workflow.',
        occurredAt: '2026-09-05T01:03:00.000Z',
        idempotencyKey: 'examination-restart-project',
        correlationId: source.correlationId
      });

      const firstReader = new ExaminationStageReadService(
        formalMatterRepository,
        lifecycleRepository
      );
      ...
      const examinationTables = await database.getPool().query(
