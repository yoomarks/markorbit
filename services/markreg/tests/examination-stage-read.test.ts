...
function event(
  ...
  return {
    ...
    occurredAt:
      input.occurredAt ??
      `2026-08-10T00:${String(version).padStart(2, '0')}:00.000Z`,
...
function viewFor(
  current: LifecycleEventProjection,
  input: { formalMatterVersion?: number } = {}
) {
  return {
    schemaVersion: 1,
...
function lifecycle(
  ...
  return {
    getCurrentView: () =>
      Promise.resolve(currentView ? structuredClone(currentView) : undefined),
...
function service(
  ...
  return new ExaminationStageReadService(
    formalMatters(),
    lifecycle(currentView, events)
  );
}

describe('Examination Stage V1 owner read', () => {
  it(
    'establishes each admitted Examination event-code/state pair without Official Truth or deadline',
    async () => {
      for (const [eventCode, state] of Object.entries(examinationEventPolicy) as [
        ...
        const result = await service(viewFor(current), [current]).get(
          workspaceId,
          formalMatterId
        );
  ...
  it(
    'fails closed when an Examination event code is paired with the wrong lifecycle state',
    async () => {
      const invalid = event(
        'invalid-policy',
  ...
  it(
    'keeps qualifying history historical when the exact current event is unrelated',
    async () => {
  ...
  it(
    'returns successful known absence only when lifecycle read succeeds with no lifecycle truth',
    async () => {
  ...
  it(
    'bounds deterministic Examination history to the latest 50 qualifying entries',
    async () => {
      const history = Array.from(
        { length: EXAMINATION_HISTORY_LIMIT + 5 },
        (_, index) =>
          event(
            `history-${index + 1}`,
      ...
      expect(result.history.at(-1)?.lifecycleEvent.id).toBe(
        history.at(-1)?.lifecycleEventId
      );
      expect(
        result.history.every((item) => item.sourceCurrentness === 'HISTORICAL')
      ).toBe(true);
    }
  );

  it(
    'treats missing or mismatched exact current-view event identity as unavailable, not absence',
    async () => {
      const current = event(
        'corrupt',
  ...
  it(
    'fails closed when the durable Formal Matter version has moved beyond lifecycle source truth',
    async () => {
      const current = event(
        'stale-matter',
        'EXAMINATION_INTERNAL_PROCESSING',
  ...
  it(
    'maps Formal Matter or lifecycle persistence failure to unavailable instead of NOT_ESTABLISHED',
    async () => {
      const failedMatter = {
  ...
  it('keeps unknown and cross-Workspace Formal Matters privacy-safe', async () => {
    await expect(
      new ExaminationStageReadService(
        formalMatters(1, false),
        lifecycle(undefined, [])
