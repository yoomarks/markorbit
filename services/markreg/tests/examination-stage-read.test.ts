...
function event(
  ...
  return {
    ...
    version,
    source: {
      reviewedSourceAdmission: { id: `reviewed-source-admission_${suffix}`, version: 1 },
      admissionFingerprintSha256: sha('a'),
      evidenceReviewDecision: { id: `evidence-review-decision_${suffix}`, version: 1 },
...
function service(currentView: CurrentLifecycleView | undefined, events: readonly LifecycleEventProjection[]) {
...
describe('Examination Stage V1 owner read', () => {
  ...
  it('fails closed when an Examination event code is paired with the wrong lifecycle state', async () => {
    ...
    await expect(service(viewFor(invalid), [invalid]).get(workspaceId, formalMatterId)).rejects.toMatchObject({
  ...
  it('keeps qualifying history historical when the exact current event is unrelated', async () => {
    ...
    const unrelated = event('unrelated-current', 'PROVIDER_EVIDENCE_REVIEWED', 'WAITING_NO_ACTION', {
      version: 2
    });
  ...
  it('treats missing or mismatched exact current-view event identity as unavailable, not absence', async () => {
    ...
    await expect(service(viewFor(current), []).get(workspaceId, formalMatterId)).rejects.toMatchObject({
    ...
    await expect(service(corruptView, [current]).get(workspaceId, formalMatterId)).rejects.toMatchObject({
      code: 'EXAMINATION_TRUTH_UNAVAILABLE',
      status: 503
    });
  });
  ...
  it('fails closed when the durable Formal Matter version has moved beyond lifecycle source truth', async () => {
    ...
    const read = new ExaminationStageReadService(formalMatters(2), lifecycle(viewFor(current), [current]));
  ...
  it('maps Formal Matter or lifecycle persistence failure to unavailable instead of NOT_ESTABLISHED', async () => {
    ...
    ).rejects.toMatchObject({ code: 'EXAMINATION_TRUTH_UNAVAILABLE', status: 503, retryable: true });
