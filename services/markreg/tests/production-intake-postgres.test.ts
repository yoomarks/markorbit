...
suite('PostgreSQL Production Intake', () => {
  ...
  it('persists exact structured Intake truth and replays across service restart', async () => {
    ...
    const persisted = rows.rows[0] as unknown as {
      input_snapshot: unknown;
      intake_record: unknown;
    };
    expect(rows.rowCount).toBe(1);
    expect(persisted.input_snapshot).toEqual(first.input);
    expect(persisted.intake_record).toEqual(first);
    ...
    const auditRow = audit.rows[0] as unknown as {
      action: unknown;
      actor_id: unknown;
