...
type EligibilityOverrides = Partial<
  Omit<
    ...
    'eligibilityId' | 'eligibilityFingerprintSha256'
  >
>;

function eligibility(overrides: EligibilityOverrides = {}): TrustedPublicEligibilityAuthorizationV1 {
...
describe('Trusted Public Exposure owner-local serve runtime', () => {
  ...
  it('requires the projection to bind the exact current eligibility identity, version and fingerprint', async () => {
    ...
    expect(await service().evaluateCurrentServe({ eligibility: current, projection: p })).toMatchObject({
      state: 'PRIVATE_NETWORK_ONLY',
      decision: 'DENY',
  ...
  it('fails closed for revoked, superseded, expired, or not-yet-effective eligibility', async () => {
    ...
    expect(
      await service().evaluateCurrentServe({ eligibility: revoked, projection: projection(revoked) })
    ...
    expect(
      await service().evaluateCurrentServe({ eligibility: expired, projection: projection(expired) })
