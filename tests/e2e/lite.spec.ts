...
type LitePage = Parameters<typeof watchPage>[0];

async function openLite(page: LitePage) {
...
async function openCustomers(page: LitePage) {
...
test('Lite Customers list is healthy on desktop @visual', async ({ page }, testInfo) => {
  ...
  const assertHealthy = await openCustomers(page);
...
test('Lite Opportunity detail is safe on mobile @visual', async ({ page }, testInfo) => {
  ...
  const opportunityRow = page.getByRole('button', {
    name: /Canada expansion/
  });
...
test('Lite retains opportunity filters after detail return', async ({ page }, testInfo) => {
  ...
  const originatingRow = page.getByRole('button', {
    name: /Canada expansion/
  });
