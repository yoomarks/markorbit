...
test.describe('M6-WP-06 private Capability Center real runtime', () => {
  test('loads, accepts exact candidate and reloads durable private projection without interception', async ({ page }) => {
    ...
    const auth = await page.request.post(`${gateway}/__test/auth/session`, { data: { fixture: 'wp06' } });
    ...
    const loadResponse = page.waitForResponse(
      (response) => response.url().endsWith('/api/lite/capability-center') && response.request().method() === 'GET'
    );
    await page.goto(`${lite}/?workspaceId=${workspaceId}#capability`);
    expect((await loadResponse).status()).toBe(200);

    await expect(page.getByRole('heading', { name: 'Capability Center', exact: true })).toBeVisible();
    await expect(page.getByText(/does not create certification, ranking, canonical truth/)).toBeVisible();
    ...
    const decision = page.waitForResponse(
      (response) => response.url().includes('/reflection-candidates/') && response.url().endsWith('/disposition') && response.request().method() === 'POST'
    );
    ...
    await expect(page.getByText(/My private Capability Ledger contains 1 governed work outcome/)).toBeVisible();
    ...
    await expect(direct.getByRole('heading', { name: 'Capability Center', exact: true })).toBeVisible();
    await expect(direct.getByText(/My private Capability Ledger contains 1 governed work outcome/)).toBeVisible();
    await direct.close();
