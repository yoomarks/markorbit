...
test('Completed Professional Review reaches an immutable Preparation Lock @visual', async ({
  ...
  const assertHealthy = watchPage(page);
  const ownerFixture = await installPreparationGatewayFixture(page);
  ...
  const requirement = page.getByRole('listitem').filter({ hasText: 'Applicant identity evidence' });
  await expect(requirement).toContainText('Missing');
  await expect(requirement).toContainText('blocking');
  ...
  await page.getByLabel('External storage/reference (optional)').fill('vault://e2e/passport');
  await page.getByRole('button', { name: 'Record evidence metadata' }).click();
  await expect(requirement).toContainText('Passport evidence · RECORDED');
  ...
  await expect(page.getByText(/document-package_e2e011 · version 4/)).toBeVisible();
  await expect(
  ...
  const lock = ownerFixture.getPreparationLock();
  expect(lock?.source.documentPackageVersion).toBe(4);
  expect(lock?.source.canonicalEvidenceHash).toBe('a'.repeat(64));
  expect(lock?.authority).toEqual({
